...
import { getAllBlockDefinitions, getBlockSettings } from '../PageBuilder/blocks'
import type { Block, SaveData } from '../PageBuilder/types'
...
const findBlockById = (blockList: Block[], id: string): Block | null => {
  for (const block of blockList) {
    ...
    // Поддержка Grid
    if (block.type === 'grid' && block.props.columnData) {
      for (const colIndex in block.props.columnData) {
        const columnBlocks = block.props.columnData[colIndex]
        if (Array.isArray(columnBlocks)) {
          const found = findBlockById(columnBlocks, id)
          if (found) return found
        }
      }
    }
...
const moveBlockUp = (id: string) => {
  const moveInList = (list: Block[]): boolean => {
    ...
    for (const block of list) {
      // Проверяем children (для Container, Link)
      if (block.children && moveInList(block.children)) {
        return true
      }
      // Проверяем columnData (для Grid)
      if (block.type === 'grid' && block.props.columnData) {
        for (const colIndex in block.props.columnData) {
          const columnBlocks = block.props.columnData[colIndex]
          if (Array.isArray(columnBlocks) && moveInList(columnBlocks)) {
            return true
          }
        }
      }
...
// ТАКЖЕ ИСПРАВИТЬ duplicateBlock и deleteBlock
  ...
  const findAndInsert = (list: Block[]): boolean => {
    ...
    for (const block of list) {
      ...
      if (block.type === 'grid' && block.props.columnData) {
        for (const colIndex in block.props.columnData) {
          const columnBlocks = block.props.columnData[colIndex]
          if (Array.isArray(columnBlocks) && findAndInsert(columnBlocks)) {
            return true
          }
        }
      }
  ...
  const removeFromList = (list: Block[]) => {
    ...
    for (const block of list) {
      ...
      if (block.type === 'grid' && block.props.columnData) {
        for (const colIndex in block.props.columnData) {
          const columnBlocks = block.props.columnData[colIndex]
          if (Array.isArray(columnBlocks) && removeFromList(columnBlocks)) {
            return true
          }
        }
      }
