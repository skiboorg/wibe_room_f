...
<template>
  ...
  <CardBase padding="lg">
    ...
    <p class="text-center md:font-medium">Найдите что то для себя и своё.</p>
    <p class="text-center md:font-medium mb-8">Текст для второй строки</p>

    <div class="space-y-3 mb-8">
      <div class="w-full flex items-center gap-3 bg-[#F1F2F4] p-4 md:px-[20px] md:py-[22px] rounded-[16px] mb-3">
        ...
        <input
            class="bg-[#F1F2F4] w-full outline-0 text-lg"
            v-model="filters.q"
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-5 mb-8">
      <CardGroup
          :community="community"
          v-for="community in community_response?.results"
          :key="community.id"
      />
    </div>
    ...
    <!-- пагинатор -->
    <UIPaginator
        v-if="community_response?.count > 10"
        :total="community_response?.count ?? 0"
        :page="filters.page"
        :per-page="10"
